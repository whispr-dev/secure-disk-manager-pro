...
## Windows build sanity command

This crate contains both a library target and an explicit binary target. The `.rlib` file is normal, but it is not the runnable program. Build the runnable executable with:
...
Build the demo executable with:
...
If Cargo reports `no bin target named secure_diskmanager_ops`, you are not in this patched crate folder or your extraction overlaid the wrong directory.

## v0.2.0 Python Demo Expansion

This version adds `src/python_ops.rs`, a std-only Rust expansion of the attached `python_library_demos_sm0l.tar.gz` scripts.

The Python archive mostly contained small third-party-library demonstrations. The Rust port preserves the useful operator behaviours without embedding heavyweight browser, OCR, API, or licensed chart engines inside the OS-operator crate.

New files:

- `src/python_ops.rs`
- `examples/python_ops_demo.rs`
- `PYTHON_DEMO_FUNCTION_MAP.md`

New CLI smoke tests:

```powershell
cargo run --bin secure_diskmanager_ops -- py-demo-list
cargo run --bin secure_diskmanager_ops -- py-demo-report
cargo run --bin secure_diskmanager_ops -- py-fuzzy kittn kitten sitting knitting bitten
cargo run --bin secure_diskmanager_ops -- py-identify test@example.com
cargo run --bin secure_diskmanager_ops -- py-chart-svg demo_chart.svg
cargo run --example python_ops_demo
```

The explicit non-ports are intentional:

- OpenAI API calls need credentials and an app-level HTTP client.
- Selenium/Playwright need browser binaries/drivers and should be app-level tools.
- Tesseract OCR needs the external Tesseract engine.
- LightningChart may require licensing, so this crate exposes renderer-neutral chart helpers instead.
